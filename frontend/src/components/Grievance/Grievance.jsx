...
const Grievance = ({ style, grievance, rejectMutation, selectMutation, addCommentMutation, author, addCommentLoading }) => {
  ...
  let { title, complaintAgainst, createdAt, description, authorId, id, supportingDocsUrls, cgrcDocsUrls, status, actionTakenByPrincipal, comments } = grievance;
  return (
    <div className="Grievance">
      ...
      <div className="ViewGrievance" style={style}>
        ...
        <div className="ViewGrievance__submit">
          ...
          {userType === "SECRETARY" && status === "NEW" && (
            <Button type="primary" icon={<CheckOutlined />} onClick={() => selectMutation(id)}>
              Select
            </Button>
          )}
          {userType === "SECRETARY" && status === "NEW" && (
            <>
              <Popconfirm title="Are you sure about rejecting this grievance?" okText="Yes" onConfirm={() => setShowRejectModal(true)} cancelText="No">
                <Button type="primary" icon={<CloseOutlined />} style={{ background: "#E5435A", borderColor: "#E5435A" }}>
                  Reject
                </Button>
              </Popconfirm>
            </>
      ...
      {(status !== "NEW" || status !== "SELECTED") && userType !== "STUDENT" && (
